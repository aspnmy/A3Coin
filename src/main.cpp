...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 18);//20
static CBigNum bnProofOfStakeLimit(~uint256(0) >> 18);//20

static CBigNum bnProofOfWorkLimitTestNet(~uint256(0) >> 18);//20
static CBigNum bnProofOfStakeLimitTestNet(~uint256(0) >> 18);//20
...
unsigned int nStakeTargetSpacing = 60;//30;			// 30 sec block spacing

int64 nChainStartTime = 1398564038;//1391393673;//1398527746
int nCoinbaseMaturity = 30;//30
...
const string strMessageMagic = "a3coin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    int64 nSubsidy = 1*CENT;

    if(nHeight == 1)
    {
        nSubsidy =  CIRCULATION_MONEY /1.25;//MAX_MONEY*0.8
        return nSubsidy + nFees;
    }
    else
        nSubsidy=1*CENT;
    //printf("0001nSubsidy= %u \n", nSubsidy);
...
const int YEARLY_BLOCKCOUNT = 365*1440;	// 365 * 2880
...
static const int64 nTargetTimespan = 30 * 60;//60 * 60;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        // The first for(;;) above does all the inexpensive checks.
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in a3coin
    bool fStrictPayToScriptHash = true; // Always active in a3coin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "a3coin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        // Genesis block
        const char* pszTimestamp = "2014-04-27 a3coin ";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = nChainStartTime;//1391393693;
        ...
        assert(block.hashMerkleRoot == uint256("5b0aed0dc79da6b5ccdca7023dfa7cddff5aaecf9692ccc4e6744177527e0920"));
